...
namespace LoggingKata
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if(lines.Length == 0)
            {
                logger.LogError("file has 0 lines");
            }
            if(lines.Length == 1)
            {
                logger.LogWarning("file has only 1 line of input");
            }
            ...
            logger.LogInfo($"{tacoBell1.Name} and {tacoBell2.Name} are the furthest apart, they are {((int)distance) / 1000} kilometers " +
